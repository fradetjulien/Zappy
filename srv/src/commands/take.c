...
static int	check_on_map_first(t_map *tmp, server *server,
char **params, int i)
{
	if (strcmp("food\n", params[1]) == 0 &&
	tmp->rsrc->nb_food > 0) {
...
static int	check_on_map_second(t_map *tmp, server *server,
char **params, int i)
{
	if (strcmp("mendiane\n", params[1]) == 0 &&
...
static int    check_on_map_third(t_map *tmp,
server *server, char **params, int i)
{
	if (strcmp("thystame\n", params[1]) == 0 &&
...
static void manage_take(server *server, t_map *tmp, int i, char **params)
{
...
int take(server *server, int i, char **params)
{
	...
	>= (7 / server->frequency)) {
		while (tmp) {
			manage_take(server, tmp, i, params);
			tmp = tmp->next;
		}
		return (0);
	}

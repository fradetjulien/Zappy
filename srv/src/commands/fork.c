...
static void delete_elem_egg(egg **eggs, egg *tmp, egg *elem)
{
	if (tmp->id == elem->id) {
...
void	new_client(server *server, egg *elem)
{
	...
	int	csock = accept(server->fd,
	(struct sockaddr *)&csin, (unsigned int *)&sinsize);
	server->fd_max = csock > server->fd_max ? csock : server->fd_max;
	client c = {csock, elem->team_name,
		0, server->nb_player, 1, elem->pos_x,
